...
void TriMesh::need_adjacentfaces()
{
	...
	vector<int> numadjacentfaces(nf);
	for (int i = 0; i < nf; i++) {
...
void TriMesh::need_four_faces()
{
	if (!face_four_neighbor.empty())
		return;
	need_faces();

	dprintf("Finding every faces' four neighbor... ");
	int nv = vertices.size(), nf = faces.size();

	face_four_neighbor.resize(nf);
	for (int i = 0; i < nf; i++)
	{
		face_four_neighbor[i].reserve(4);
		face_four_neighbor[i].push_back(i);
	}

	for (int i = 0; i < nf; i++) {
		for (int j = 0; j < 3; j++)
			face_four_neighbor[faces[i][j]].push_back(i);
	}
