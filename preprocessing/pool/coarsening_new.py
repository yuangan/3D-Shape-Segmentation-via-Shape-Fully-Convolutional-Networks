...
def coarsen(A, levels, self_connections=False):
    ...
    return graphs, perms,fake_number
...
def metis(W, levels, rid=None):
    ...
    for _ in range(levels):
        ...
        weights = np.array(weights).squeeze()#del one degree
...
def metis_one_level(rr,cc,vv,rid,weights):#pooling cluster divided by 2,one vertex and it's neighbor are one cluster
    ...
    for ii in range(N):
        tid = rid[ii]#random permutation,vertex order
        try:
            a=marked[tid]
        except:
            print N
...
def compute_perm(parents):
    ...
    for parent in parents[::-1]:#order reverse [::-1]
