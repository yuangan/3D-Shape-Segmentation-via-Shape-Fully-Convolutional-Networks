...
void GCoptimization::setupDataCostsExpansion(SiteID size,LabelID alpha_label,EnergyT *e,SiteID *activeSites)
{
	...
	for (SiteID i = 0; i < size; ++i){
		//printf("%f\n",m_labelingDataCosts[activeSites[i]]);
...
void GCoptimization::setupSmoothCostsExpansion(SiteID size,LabelID alpha_label,EnergyT *e,SiteID *activeSites)
{
	...
	for ( i = size - 1; i >= 0; i-- )
	{
		...
		for ( n = 0; n < nNum; n++ )
		{
			if (weights[n]>1000000 || weights[n] < -1000000){
				printf("%d ----------------------------------", n);
				exit(-1);
			}
			nSite = nPointer[n];
...
bool GCoptimization::alpha_expansion(LabelID alpha_label)
{
	...
	SiteID size = 0;
	SiteID *activeSites = new SiteID[m_num_sites];//all points
	...
	try 
	{
		...
		//printf("%d, %f, %f, %d, %d\n", alpha_label, m_beforeExpansionEnergy, afterExpansionEnergy, m_setupDataCostsExpansion, m_setupSmoothCostsExpansion);
		...
		checkInterrupt();
		//printf("%f", alphaCorrection);
		afterExpansionEnergy = e.minimize() + alphaCorrection;

		//printf("%d, %f, %f, %f, %f\n",alpha_label, e.minimize(), m_beforeExpansionEnergy, afterExpansionEnergy, afterExpansionEnergy-m_beforeExpansionEnergy);
		checkInterrupt();
